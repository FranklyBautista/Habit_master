...
- [ ] Preparar iconos, splash screen, permisos y textos de privacidad. (2026-09-23: **iconos** — el mismo check blanco sobre verde `#047857` de la PWA sustituye al logo de Expo en `icon.png` (también `ios.icon`), las tres capas del icono adaptativo de Android (fondo verde, check dentro de la zona segura, monocromo para iconos temáticos) y `favicon.png`; **splash** verde de marca con el check (variante oscura `#064E3B`); **notificaciones** con `notification-icon.png` monocromo, color de marca y canal `reminders` por defecto; **permisos** — de los 5 que declaraba el build solo quedan `INTERNET` y `VIBRATE`, se bloquean `SYSTEM_ALERT_WINDOW` y `READ`/`WRITE_EXTERNAL_STORAGE` (verificado con `expo config --type introspect`); **privacidad** — página pública `/privacidad` en la web (sin sesión, excluida de la redirección de `proxy.ts`), enlazada desde Ajustes en web y en móvil (móvil abre `https://habit-master-web.vercel.app/privacidad`, la misma URL que se declarará en las tiendas). Cubierto por E2E en `pwa.spec.ts` y axe en `accessibility.spec.ts`. Contacto: `habitmaster67@gmail.com`. Pendiente: desplegar la web para que `/privacidad` exista en producción y revisar icono/splash/notificación en el Android real con el próximo development build.)
