...
- [x] Reutilizar `domain`, tipos de base de datos y design tokens. `@habit-tracker/domain` y los tipos de `@habit-tracker/database` se reutilizan en todas las pantallas. **Cambio de alcance (2026-09-16):** se descarta crear `packages/design-tokens` compartido entre web y móvil. Al comparar las paletas (`apps/mobile/src/constants/theme.ts` vs `apps/web/src/app/globals.css`) resultó que ya divergieron: solo el verde de marca (`tint`/`primary` = `#047857`) coincide a propósito; el resto usa paletas distintas por diseño — web la escala "slate" de Tailwind, móvil grises neutros más propios de una app nativa. Compartir tokens de verdad obligaría primero a elegir una paleta ganadora y repintar una de las dos plataformas (con su correspondiente re-verificación de contraste/accesibilidad), y en el lado web exigiría además un paso de build para generar las variables CSS desde constantes TS — Tailwind no puede importar JS directo en `globals.css`. Es justo el riesgo que la sección 10 ya advierte ("Forzar una UI universal difícil de mantener"), desproporcionado para un proyecto personal. Móvil sigue con `src/constants/theme.ts` + `components/card.tsx` propios; la única regla que se mantiene es sincronizar el verde de marca a mano entre ambas paletas si alguna vez cambia. Se reevaluará si el proyecto suma colaboradores o si la paridad visual exacta entre plataformas se vuelve un requisito real.
...
- [x] Adaptar componentes a controles nativos y accesibles. (2026-09-16: `hitSlop` en los botones de icono de `habitos.tsx` para llegar a 44×44pt/48×48dp sin agrandar el layout, `accessibilityRole`/`accessibilityLabel` en los botones primarios de auth y del modal de hábito, y `accessibilityState={{ disabled }}` en todos los controles deshabilitables. Ver `EXTRAS_IMPROVEMENT` §Mobile — accesibilidad.)
...
- [x] Añadir recordatorios locales configurables después de estabilizar el tracking. (2026-09-16: implementado con `expo-notifications` — toggle + 4 horarios preestablecidos en Ajustes, canal de Android, permisos solicitados al activar, preferencia guardada solo en el dispositivo con `AsyncStorage` (no se sincroniza por Supabase: cada instalación necesita su propio recordatorio). 2026-09-23, revisión: el canal `reminders` se creaba pero el trigger no pasaba `channelId` (caía en el canal genérico de Android) — corregido; el canal ahora se crea **antes** de pedir el permiso (Android 13+ no muestra el diálogo sin un canal existente); fallos al programar ya no son promesas sin capturar sino un error visible que no deja el toggle en un estado falso; el toggle se muestra apagado si el permiso se revocó desde el sistema; la sección se oculta en Expo web (no soporta notificaciones locales). Cubierto por `src/lib/notifications/reminders.test.ts` (8 casos). Pendiente: `expo-notifications` es un módulo nativo, así que hace falta un **nuevo** `eas build --profile development --platform android` e instalarlo; luego verificar en el Android real que aparece el diálogo de permiso, que llega la notificación a la hora elegida (con la app cerrada), que cambiar la hora no duplica avisos y que desactivarlo la cancela. **Verificado 2026-09-24** en Android real con el development build `7e68282f`: todo lo anterior funciona. 2026-09-24, ampliación: **varios recordatorios** (hasta 5, lista con editar/eliminar en Ajustes; la preferencia antigua de uno solo se migra) y **hora personalizada** en un modal con campos para escribirla a mano (12 h + a. m./p. m., validada), botones −/+ y las 4 horas como atajos; los cambios se aplican en cola para no dejar programada una hora vieja. **Hallazgo:** con la hora personalizada el aviso no llegó — la app no declaraba `SCHEDULE_EXACT_ALARM`, así que en Android 12+ `expo-notifications` usa una alarma inexacta que el sistema puede retrasar mucho; se añade el permiso en `app.json` y un aviso en Ajustes para activar «Alarmas y recordatorios» (en Android 14+ viene desactivado por defecto). Cubierto por `reminder-time.test.ts`, `reminders.test.ts` y `reminder-time-modal.test.tsx`. Pendiente: nuevo development build (el permiso es nativo) y verificar en el dispositivo.)
...
- [x] Preparar iconos, splash screen, permisos y textos de privacidad. (2026-09-23: **iconos** — el mismo check blanco sobre verde `#047857` de la PWA sustituye al logo de Expo en `icon.png` (también `ios.icon`), las tres capas del icono adaptativo de Android (fondo verde, check dentro de la zona segura, monocromo para iconos temáticos) y `favicon.png`; **splash** verde de marca con el check (variante oscura `#064E3B`); **notificaciones** con `notification-icon.png` monocromo, color de marca y canal `reminders` por defecto; **permisos** — de los 5 que declaraba el build solo quedan `INTERNET` y `VIBRATE`, se bloquean `SYSTEM_ALERT_WINDOW` y `READ`/`WRITE_EXTERNAL_STORAGE` (verificado con `expo config --type introspect`); **privacidad** — página pública `/privacidad` en la web (sin sesión, excluida de la redirección de `proxy.ts`), enlazada desde Ajustes en web y en móvil (móvil abre `https://habit-master-web.vercel.app/privacidad`, la misma URL que se declarará en las tiendas). Cubierto por E2E en `pwa.spec.ts` y axe en `accessibility.spec.ts`. Contacto: `habitmaster67@gmail.com`. Web desplegada (push a `main`, `/privacidad` responde en producción). Pendiente: revisar icono/splash/notificación en el Android real con el development build `7e68282f` (lanzado 2026-09-23 desde `c281451`). **Verificado 2026-09-24** en Android real con ese build: icono y splash de marca, icono de notificación y enlace a `/privacidad`.)
- [ ] Hacer una beta interna antes de preparar App Store/Play Store.

**Criterio de salida — v1 móvil**

- [x] Marcar un hábito en móvil actualiza el estado visible en web y viceversa, sin duplicados ni pérdida de historial. (Verificado 2026-09-16 con la misma cuenta en Android real + web local: check-ins idempotentes ante toques repetidos, sin duplicados en `habit_checkins`; el bug de archivar/restaurar que corrompía el historial de rachas se corrigió el 2026-09-24, ver `EXTRAS_IMPROVEMENT` §Seguridad y datos.)
...
- [x] **Enumeración de usuarios en registro web.** `apps/web/src/app/(auth)/actions.ts:36-38` — cuando Supabase devuelve `"User already registered"`, `register()` respondía `"Ya existe una cuenta con este correo."`, revelando si un email tiene cuenta. Contradecía el propio diseño anti-enumeración de `recoverPassword()` (mensaje deliberadamente ambiguo). **Corregido (2026-09-23):** `register()` (web) y el flujo de registro móvil (`apps/mobile/src/app/(auth)/registro.tsx`, mismo bug duplicado en `apps/mobile/src/lib/auth/errors.ts`) ahora devuelven el mismo mensaje genérico "Revisa tu correo para confirmar la cuenta." sin importar si la cuenta ya existía. Verificado en navegador y en Android real.
...
- [x] **Archivar y restaurar un hábito corrompe retroactivamente sus rachas.** El dominio solo guardaba un `archivedAt` único (`isHabitActiveOnDate` en `dates.ts`); al restaurar (`archivedAt: null`) el hábito volvía a contarse activo desde `startDate`, incluyendo los días en que estuvo archivado, rompiendo tanto la racha previa como el cumplimiento del periodo. **Corregido (2026-09-24):** nueva tabla `habit_archive_periods` (migración `20260924120000`) con las ventanas ya cerradas `[archived_at, restored_at)`; la rellena un trigger `security definer` al pasar `archived_at` de un valor a `null`, en la misma transacción y con la hora del servidor — los clientes solo tienen `SELECT` (RLS owner-only), así que no pueden inventar periodos para ocultar días incumplidos. El dominio añade `Habit.archivePeriods` (`default([])`, compatible con estados guardados antes) e `isHabitActiveOnDate` excluye cada ventana (el día de archivado no cuenta, el de restauración sí). Web y móvil la leen en `getState`; el adaptador local registra el periodo igual que el trigger. Verificado con 7 tests nuevos de dominio, 13 pgTAP nuevos (41 en total), tests de repositorio web/móvil, las 20 E2E y una prueba contra la base local real (racha 4 con historial vs. 1 con el bug). Los hábitos restaurados **antes** de esta migración no recuperan su historial (no se guardaba). **Despliegue:** aplicar la migración en producción (`supabase db push`) **antes** de publicar el código — si no, `getState` falla al consultar una tabla que no existe.
...
### Mobile — accesibilidad (bloqueaba el checkbox "controles nativos y accesibles" de Fase 8 — resuelto 2026-09-16)

- [x] **Áreas táctiles por debajo del mínimo.** `apps/mobile/src/app/(app)/habitos.tsx` — los botones Subir/Bajar/Editar/Archivar/Restaurar (icono de 16–20px, `padding: Spacing.one`) ahora usan `hitSlop={{top:10,bottom:10,left:10,right:10}}` para llegar a 44×44pt/48×48dp sin cambiar el layout visual de la fila.
- [x] **Botones primarios sin `accessibilityRole`/`accessibilityLabel`.** Agregado en `habit-form-modal.tsx` (Cancelar/Guardar), `login.tsx`, `registro.tsx`, `recuperar.tsx` y `actualizar-contrasena.tsx`.
- [x] **Controles deshabilitados sin `accessibilityState={{ disabled: true }}`.** Agregado en las flechas de `habitos.tsx`, `connection-banner.tsx` y el botón "Guardar cambios" de `ajustes.tsx`.

### Mobile — contraste de formularios de auth (encontrado y resuelto 2026-09-23)

- [x] **Inputs ilegibles en modo oscuro.** `apps/mobile/src/lib/auth/form-styles.ts` — `authFormStyles.input` no definía `color` ni `backgroundColor`; el `TextInput` caía al negro por defecto de Android sin fondo propio, invisible sobre el `ThemedView` oscuro de las pantallas de login/registro/recuperar/actualizar-contraseña. Se agregó `useAuthInputStyle()` (colores del tema activo vía `useTheme()`) y se aplicó en las 4 pantallas.
